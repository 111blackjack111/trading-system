...
12. **Новостной фильтр (ForexFactory / MQL5 Calendar)**
    ForexFactory.com — календарь экономических событий.
    Два режима:
    Режим 1 (защитный): не торговать ±30 мин от High Impact новостей (NFP, Rate Decision).
    Режим 2 (торговый): торговать импульс после выхода новости по FVG.
    Реализация: agents/news_filter_agent.py скачивает календарь каждое утро в 08:00 UTC+3.
    pip install market-calendar-tool для ForexFactory скрапинга.
    Для бэктеста: MQL5 Economic Calendar API (исторические данные).
    Параметр: news_buffer_minutes = 30 (оптимизируемый, диапазон 15-60).
    Статус: реализовать ПОСЛЕ 50 итераций текущего цикла, ПЕРЕД paper trading.

Статус гипотез: параметрическая оптимизация завершена (score 4.28).
Сейчас: 50 итераций с ослабленными фильтрами для увеличения сделок.
